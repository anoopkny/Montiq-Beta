...
<form action="" method="post">
    ...
    <div class="sel-users-new" style="width: 400px; margin-bottom: 20px;">
        ...
        foreach ($xml->user as $user) {
            if (strval($user->username) == $curr_username) { continue; }
            $nusers[strval($user->name)] = array('username' => strval($user->username),
                                         'id' => intval($user->attributes()->id));
        }
        ksort($nusers);

        // Generate the div box with all the users inside
        foreach ($nusers as $fullname => $user) {
            ...
            echo "<div class='checkbox'><label><input type='checkbox' name='users[" . $user['id'] . "]' " . $ischecked . ">" . $fullname . " (" . $user['username'] . ")</label></div>";
