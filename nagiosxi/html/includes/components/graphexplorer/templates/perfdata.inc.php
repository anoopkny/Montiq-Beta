<?php
// perfdata.inc.php
// - Template file for highcharts perfdata graphs     
...
function fetch_timeline_perfdata($args)
{
    ...
    $hc_show_rrd_stats = get_option('hc_show_rrd_stats', array('avg', 'min', 'last'));
    ...
    for ($i = 0; $i < count($args['names']); $i++) {
        $uom_array[] = "'".$args['names'][$i]."': '".$units[$i]."'";
    }
                    ...
                    tooltip: {
                        ...
                        formatter: function() {
                            ...
                            for (var i = 0; i < this.points.length; i++) {
                                ...
                                html += '<br><span style="color:' + this.points[i].series.color + '">\u25CF</span> <b>' + name + '</b>: ' + value;
                            }
                            return html;
                        }
                    },
...
// End heredoc syntax
    ...
    for($i=0;$i<count($args['datastrings']);$i++)
    {
        ...
        if (!empty($hc_show_rrd_stats)) {
            ...
            if (in_array('last', $hc_show_rrd_stats)) {
                $vars[] = _('Last') . ': ' . round($perfdata_extra[$i]['last'], 2).$units[$i];
            }

            if (in_array('avg', $hc_show_rrd_stats)) {
                $vars[] = _('Avg') . ': ' . round($perfdata_extra[$i]['avg'], 2).$units[$i];
            }

            if (in_array('max', $hc_show_rrd_stats)) {
                $vars[] = _('Max') . ': ' . round($perfdata_extra[$i]['max'], 2).$units[$i];
            }
    ...
    function addBands(chartid, type, addPlot, extremes) {
        ...
        if (type === 1) {
            ...
            if (!uchart.resetZoomButton) {
                uchart.showResetZoom();
            }
        } else {
